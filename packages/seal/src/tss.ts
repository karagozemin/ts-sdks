...
class Polynomial {
	...
	static monic_linear(c: GF256): Polynomial {
		return new Polynomial([new GF256(0).sub(c), new GF256(1)]);
	}
	...
	static interpolate(x: GF256[], y: GF256[]): Polynomial {
		...
		const n = x.length;
		let sum = Polynomial.zero();
		for (let j = 0; j < n; j++) {
			let product = new Polynomial([new GF256(1)]);
			for (let i = 0; i < n; i++) {
				if (i !== j) {
					product = product.mul(Polynomial.monic_linear(x[i]).div(x[j].sub(x[i])));
				}
			}
			sum = sum.add(product.scale(y[j]));
		}
		return sum;
	}
...
export function interpolate(
	shares: { index: number; share: Uint8Array }[],
): (x: number) => Uint8Array {
	if (shares.some(({ share }) => share.length !== 32)) {
		throw new Error('Invalid input length');
	}

	const polynomials = Array.from({ length: 32 }, (_, i) =>
		Polynomial.interpolate(
			shares.map(({ index }) => new GF256(index)),
			shares.map(({ share }) => new GF256(share[i])),
		),
	);
	...
	return (x: number) => {
		...
		for (let i = 0; i < 32; i++) {
			result[i] = polynomials[i].evaluate(new GF256(x)).value;
		}
		return result;
	};
}

...
- Users will only be able to connect their zkLogin wallet from Slush.
- The zkLogin account will be managed by Slush, you do not need to set up any OAuth providers.
...
To use the Slush wallet, you will need to register it in your application, using
...
- `options`: An optional object with the following properties:
  - `origin`: The origin of the Stashed website. Defaults to `https://getslush.com`.
...
function StashedOnly() {
	...
	const walletIsStashedWallet = currentWallet?.name === SLUSH_WALLET_NAME;
...
If you're using dapp-kit, update to the latest dapp-kit version and change the
'stashedWallet`prop on WalletProvider to`slushWallet`

...
function createSessionStorageKey(apiKey: string, network: EnokiNetwork, providerId: string) {
...
export class INTERNAL_ONLY_EnokiFlow {
	...
	#store: SyncStore;
	#sessionStateByNetwork: Map<EnokiNetwork, EnokiSessionState>;
	...
	constructor(config: EnokiFlowConfig) {
		...
		this.#sessionStateByNetwork = this.#clients.reduce((accumulator, client) => {
			const network = client.network as EnokiNetwork;
			const storageKey = createSessionStorageKey(config.apiKey, network, config.providerId);
			const storePrefix = `${config.apiKey}_${network}`;
			const idbStore = createStore(`${storePrefix}_db`, `${storePrefix}_store`);
		...
		try {
			const rawStoredValue = this.#store.get(this.#stateStorageKey);
			if (rawStoredValue) {
		...
		onSet(this.#zkLoginState, ({ newValue }) => {
			this.#store.set(this.#stateStorageKey, JSON.stringify(newValue));
		});
	}
	...
	async #setSession(state: EnokiSessionState, newValue: ZkLoginSession | null) {
		if (newValue) {
			...
			this.#store.set(state.storageKey, storedValue);
		} else {
			this.#store.delete(state.storageKey);
		}
	...
	async #getSession({ $zkLoginSession, storageKey }: EnokiSessionState) {
		...
		try {
			const storedValue = this.#store.get(storageKey);
	...
	async logout() {
		this.#zkLoginState.set({});
		this.#store.delete(this.#stateStorageKey);
	...
	async getProof(input: { network: EnokiNetwork }) {
		const state = this.#sessionStateByNetwork.get(input.network);
		if (!state) throw new Error(`The network ${input.network} isn't supported.`);
	...
	async getKeypair(input: { network: EnokiNetwork }) {
		const state = this.#sessionStateByNetwork.get(input.network);
		if (!state) throw new Error(`The network ${input.network} isn't supported.`);

...
export function init(packageAddress: string) {
    ...
    function fresh_proposal(options: {
        ...
        const argumentsTypes = [
            "u32",
            `${packageAddress}::upgrade::PackageDigest`,
            "u64"
        ];
        return (tx: Transaction) => tx.moveCall({
            package: packageAddress,
    ...
    return { _new, vote_for_upgrade, authorize_upgrade, authorize_emergency_upgrade, commit_upgrade, cleanup_upgrade_proposals, burn_emergency_upgrade_cap, fresh_proposal, add_vote };
}